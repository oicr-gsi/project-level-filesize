...
my $dbh =
  DBI->connect( "dbi:Pg:service=prod", undef, undef, { AutoCommit => 1 } )
...
open my $PROJECT_FH, "<", $ProjectFile
  or die "Can't read file '$ProjectFile'\n";
...
open my $OUTPUT_FILE_FH, ">", $OutputFile
  or die "Can't create file '$OutputFile'\n";

my $FileSizeSum = 0;    # Total size of all files in a directory

# Print header of output file
print $OUTPUT_FILE_FH
  "Date Recorded,File Path,File Size Sum (Bytes),Quota (TB)\n";
...
while (<$ALL_DIR_FH>) {
    ...
    $Quota =
`/oicr/local/sw/hpcquota/hpcquota-1.0.1/bin/hpcquota -d "$IFSPath" --no-header --units=GiB 2>/dev/null | sed 's/ \\+/\\t/g' | cut -f3`;
    chomp($Quota);
